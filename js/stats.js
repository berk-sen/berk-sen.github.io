async function loadStats(){
  ...
  const totals={}, rounds={}, timeline={}, weekly=[]; 
  let dates=[];

  for(const f of files){
    const g = await (await fetch("./data/"+f)).json();
    const r = Object.values(g.players)[0].length; 
    dates.push(g.date);
  ...
  // Haftalık kazananlar tablosu
  html+=`<h2>Weekly Biggest Winners</h2><table><thead><tr><th>Date</th><th>Winner</th><th>Earnings</th></tr></thead><tbody>`;
  weekly.forEach(w=>{
    html+=`<tr${w.player ? ' class="highlight"' : ''}><td><a href="game.html?file=${w.file}">${w.date}</a></td><td>${w.player}</td><td>${w.score}</td></tr>`;
  });
  html+=`</tbody></table>`;
  ...
  const datasets=Object.keys(timeline).map(p=>({
    label:p,
    data:timeline[p],
    fill:false,
